...
exprToString (Cube e)             = exprToString e ++ "*" ++ exprToString e ++ "*" ++ exprToString e
exprToString (Mean e1 e2 e3)      = "((" ++ exprToString e1 ++  "+" ++ exprToString e2 ++ "+" ++ exprToString e3 ++ ")/3)"
-------------------------------------------------------------------------------
...
eval x y VarX = x
eval x y VarY = y
eval x y (Sine e) = sin(3.142 * (eval x y e)) 
eval x y (Cosine e) = cos(3.142 * (eval x y e))
...
eval x y (Cube e) = (((eval x y e) * (eval x y e)) * (eval x y e))
eval x y (Mean e1 e2 e3) = (((eval x y e1) + (eval x y e2) +(eval x y e3))/3)
...
build d 
  | r == 0 = Sine (build (d-1))
  | r == 1 = Average (build (d-1)) (build (d-1)) 
  | r == 2 = Sine (build (d-1))
  | r == 3 = Cosine (build (d-1))
  | r == 4 = Times (build (d-1)) (build (d-1)) 
  | otherwise = Cube (build (d-1)) 
  where 
    r         = rand 5
--------------------------------------------------------------------------------
-- | Best Image "Seeds" --------------------------------------------------------
